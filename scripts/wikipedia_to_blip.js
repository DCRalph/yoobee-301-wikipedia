...
function processLinks(text, links) {
  ...
  links?.forEach((link) => {
    try {
    const target = `/wiki/${slugify(link.page)}`
    const display = (link.text || link.page || 'unknown').replace(/_/g, ' ') // replace underscores with spaces
    processed = processed.replace(
      new RegExp(escapeRegExp(link.text || link.page || 'unknown'), 'g'),
        `[${display}](${target})`
      )
    } catch (e) {
      console.error('Error processing link:', link, text, e)
    }
  })
  return processed
}
...
function escapeRegExp(string) {
  try {
    ...
    console.error('Error escaping RegExp:', string, e)
    return string
  }
}
...
function processImage(img) {
  ...
  const cleanFilename = originalFilename
    ...
    .replace(
      /([([{])(\s+|_+)|(\s+|_+)([)\]}])/g,
      (_, open, after, before, close) => open || close
    )
    .replace(/ /g, '_')
    .replace(/_+/g, '_')
...
async function convertWikiToMD(wikiText) {
  ...
  for (const section of json.sections || []) {
    if (section.infoboxes?.length && !infobox) {
      infobox = section.infoboxes.find(
        (box) => Object.keys(box).length > 1 && !box.child && !('embed' in box)
      )
    }
  ...
  if (infobox) {
    ...
    for (const [key, value] of Object.entries(infobox)) {
      if (excludeKeys.has(key) || typeof value !== 'object' || !value.text)
        continue
